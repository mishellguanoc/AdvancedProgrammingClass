...
cs106l::unique_ptr<ListNode<T>> create_list(const std::vector<T>& values) {
  ...
  // Recorremos el vector en orden inverso
  for (auto it = values.rbegin(); it != values.rend(); ++it) {
    auto new_node = cs106l::make_unique<ListNode<T>>(*it); // Crear nuevo nodo
    new_node->next = std::move(head); // Conectar el nuevo nodo al anterior
    head = std::move(new_node); // Nuevo nodo se convierte en el nuevo head
  }
